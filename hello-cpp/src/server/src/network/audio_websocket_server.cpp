...
AudioWebSocketServer::AudioWebSocketServer(int port, const std::string &path)
    : port_(port), path_(path), running_(false), connectionCounter_(0) {
  initializeServer();
...
AudioWebSocketServer::~AudioWebSocketServer() { stop(); }

void AudioWebSocketServer::initializeServer() {
  try {
...
void AudioWebSocketServer::start() {
  try {
    spdlog::info("Starting AudioWebSocketServer on port {}", port_);
...
bool AudioWebSocketServer::isRunning() const { return running_; }

void AudioWebSocketServer::onOpen(ConnectionHdl hdl) {
  try {
    ...
    std::string connectionId = "conn-" + std::to_string(connectionCounter_++);
    json connectedMsg = {
        {"type", "CONNECTED"},
        {"streamId", connectionId},
        {"message", "Connection established"}
    };
    sendTextMessage(hdl, connectedMsg.dump());
    spdlog::info("Sent CONNECTED message to client: {}", connectionId);
  } catch (const std::exception &e) {
    spdlog::error("Error handling connection open: {}", e.what());
  }
}

void AudioWebSocketServer::onClose(ConnectionHdl hdl) {
...
void AudioWebSocketServer::onFail(ConnectionHdl hdl) {
  try {
    auto con = server_.get_con_from_hdl(hdl);
    auto ec = con->get_ec();
...
void AudioWebSocketServer::handleTextMessage(ConnectionHdl hdl,
...
void AudioWebSocketServer::handleBinaryMessage(ConnectionHdl hdl,
...
std::string AudioWebSocketServer::getConnectionId(ConnectionHdl hdl) const {
  try {
...
void AudioWebSocketServer::sendTextMessage(ConnectionHdl hdl,
                                      const std::string &message) {
  try {
...
void AudioWebSocketServer::sendBinaryMessage(ConnectionHdl hdl,
                                        const std::vector<uint8_t> &data) {
  try {
...
void AudioWebSocketServer::sendErrorMessage(ConnectionHdl hdl,

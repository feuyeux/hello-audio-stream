...
import { StreamContext, StreamStatus } from "./StreamContext.js";
import { MemoryMappedCache } from "./MemoryMappedCache.js";
...
export class StreamManager {
   ...
   *
   * @param {string} streamId - Unique identifier for the stream
   * @returns {Promise<StreamContext|null>} Stream context or null
   */
  ...
  async writeChunk(streamId, data) {
    ...
    try {
      ...
      if (written > 0) {
        ...
        console.debug(`Wrote ${written} bytes to stream ${streamId}`);
        return true;
      } else {
  ...
  async readChunk(streamId, offset, length) {
    ...
    try {
      ...
      console.debug(`Read ${data.length} bytes from stream ${streamId}`);
      return data;
    } catch (error) {
  ...
  async finalizeStream(streamId) {
    ...
    if (stream.getStatus() !== StreamStatus.UPLOADING) {
      console.warn(`Stream ${streamId} is not in uploading state`);
      return false;
    }

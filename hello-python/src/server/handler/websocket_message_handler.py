...
class WebSocketMessageHandler:
    ...
    async def handle_binary_message(self, websocket: ServerConnection, data: bytes):
        ...
        if not self.stream_manager.write_chunk(stream_id, data):
            logger.error(f"Failed to write {len(data)} bytes to stream {stream_id}")
            await self.send_error(
                websocket, f"Failed to write data to stream: {stream_id}"
            )
    ...
    @staticmethod
    def connected(
        connection_id: str, message: str = "Connection established"
    ) -> WebSocketMessage:
        """
        ...
        Returns:
            WebSocketMessage with CONNECTED type
        """
        return WebSocketMessage.connected(connection_id, message)
